package moye.sinetoolbox.Activity.webview;
...
public class WebviewActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public void onPageFinished(WebView view, String url) {
                try {
                    ...
                    if (history_json.length() == 0 || (history_json.length() > 0 && !history_json.getJSONObject(0).getString("url").equals(url))) {
            ...
            try {
                if (b) {
                    ...
                    if (!flags) {
                        ...
                        try {
                            for (int j = 0; j < bookmark_json.length(); j++)
                                create_bookmark_button(bookmark_json.getJSONObject(j).getString("url"), bookmark_json.getJSONObject(j).getString("title"), j);
                        } catch (JSONException e) {
                            throw new RuntimeException(e);
                        }
                    }
        ...
        findViewById(R.id.activity_webview_bookmark_back).setOnClickListener(view -> launcher.setToScreen(1));
        findViewById(R.id.activity_webview_history_back).setOnClickListener(view -> launcher.setToScreen(1));
    ...
    private void create_history_button(String history, String title, String time, int index) {
        View view = LayoutInflater.from(this).inflate(R.layout.item_history_btn, history_btns, false);
        ((TextView) view.findViewById(R.id.title)).setText(title);
        ((TextView) view.findViewById(R.id.content)).setText(history);
    ...
    private void create_bookmark_button(String url, String title, int index) {
        View view = LayoutInflater.from(this).inflate(R.layout.item_history_btn, history_btns, false);
        ((TextView) view.findViewById(R.id.title)).setText(title);
        ((TextView) view.findViewById(R.id.content)).setText(url);
